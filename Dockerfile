...
COPY . .
# main.go embeds web/dist for source-build compatibility. Production serves the
# dashboard from Cloudflare Pages, so the backend image only carries an empty
# placeholder required by go:embed instead of the compiled frontend bundle.
